...
function validateCheckoutForm() {
  const requiredFields = [
    { id: 'co-firstname', name: 'First Name' },
    { id: 'co-lastname', name: 'Last Name' },
    { id: 'co-address', name: 'Address' },
    { id: 'co-city', name: 'City' },
    { id: 'co-zip', name: 'ZIP Code' },
    { id: 'co-email', name: 'Email Address' },
    { id: 'co-phone', name: 'Phone Number' },
    { id: 'co-card', name: 'Card Number' },
    { id: 'co-expiry', name: 'Expiry Date' },
    { id: 'co-cvv', name: 'CVV' }
  ];
...
function renderCheckoutSummary() {
  const summaryPanel = document.querySelector('.checkout-order-panel');
  if (!summaryPanel) return;

  if (cart.length === 0) {
    summaryPanel.innerHTML = `
      <h3>Order Summary</h3>
      <p style="color:#94a3b8;font-size:0.88rem;">Your cart is empty.</p>`;
    return;
  }
  ...
  cart.forEach(item => {
    ...
    const subtotal = (parseFloat(unitPrice) * item.qty).toFixed(2);
    itemsHTML += `
      <div style="margin-bottom:12px;font-size:0.9rem;border-bottom:1px solid #e2e8f0;padding-bottom:8px;">
        <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
          <strong>${item.product.name}</strong>
          <span>$${subtotal}</span>
        </div>
        <div style="color:#64748b;font-size:0.85rem;">Qty: ${item.qty} @ $${unitPrice} each</div>
      </div>`;
  });
